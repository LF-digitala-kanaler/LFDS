...
const MenuItem = ({ item, location }) => {
  ...
  const renderSubMenuItems = (items) => {
    return items.map((item) => {
      return (
        <li className={style.Menu__item} key={item.node.id}>
          <MenuLink
  ...
  const hasCategory = !(
    item.parentLink === 'null' || item.parentLink === 'NoCategory'
  )
  ...
  return hasCategory ? (
    <li key={item.childLink[0].node.id} className={className}>
      ...
      />
      <ul className={style.Menu__sub}>{renderSubMenuItems(item.childLink)}</ul>
    </li>
  ) : (
