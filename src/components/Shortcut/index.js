...
const Shortcut = ({ text, path, icon, title, horizontal }) => {
  return (
    <article
      className={cx(
        style.Shortcut,
        horizontal ? style['Shortcut--horizontal'] : ''
      )}
    >
      <Link to={path} title={title} className={style.Shortcut__link}>
        ...
        {horizontal && (
          <svg
            className={style.Shortcut__arrow}
            width="21"
            height="12"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M15.207.293L20.914 6l-5.707 5.707-1.414-1.414L17.085 7H0V5h17.085l-3.292-3.293L15.207.293z"
              fill="#222"
              fillRule="evenodd"
            />
          </svg>
