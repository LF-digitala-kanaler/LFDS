...
1. Let the users fill out the form without interrupting them with error messages. \
   * 
2. When the user submits the form any potential error messages are shown to them.
3. The user is automatically scrolled to the topmost error (if several, otherwise the error). 
...
* If a field has a validation error due to the point above (that is, not having a value at all), the validation error message is removed when the user begins to fill out the input field. Validation then follows the "regular" validation as specified in the first point.
